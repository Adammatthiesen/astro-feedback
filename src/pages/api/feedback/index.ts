...
import { db, Websites, Feedback, FeedbackCategories, AnalyticsEvents } from 'astro:db';
import { eq, and, desc, asc, sql } from 'astro:db';
import { feedbackSubmissionSchema, feedbackQuerySchema } from '../../../lib/schemas';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Insert feedback
    const feedback = await db.insert(Feedback).values({
...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    let conditions = [eq(Feedback.websiteId, websiteId)];
