import { and, db, desc, eq, Feedback, FeedbackComments, Websites } from 'astro:db';
...
export const GET: APIRoute = async ({ params, request, url }) => {
	try {
		...
		const feedback = await db.select().from(Feedback).where(eq(Feedback.id, feedbackId)).get();
		...
		const conditions = [eq(FeedbackComments.feedbackId, feedbackId)];
